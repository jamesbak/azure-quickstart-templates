...
if [ -z "${PID}" ]; then
    ...
    $RUN_JAVA -server -Djava.util.logging.config.file=$HAZELCAST_HOME/bin/logging.properties -Dhazelcast.config=$HAZELCAST_HOME/bin/hazelcast.xml $JAVA_OPTS com.hazelcast.core.server.StartServer &
    echo $! > ${PID_FILE}
else
